...
def load_prompt_template() -> str:
    ...
    if "{task_instruction}" not in template:
        raise SystemExit("下载的 prompt 模板缺少 {task_instruction} 占位符，已拒绝写入本地文件。")
...
def prepare_tiptop_image(image_path: Path) -> tuple[Image.Image, bytes]:
    ...
    try:
        ...
        print(f"图像预处理完成：PIL RGB，{image.width}x{image.height}，PNG 编码。")
...
def draw_grounding_result(
    ...
    """Draw normalized OmniGround boxes directly on the TiPToP-preprocessed image."""
    ...
    draw.rectangle((4, 4, title_box[2] + 8, title_box[3] + 8), fill=(0, 0, 0))
    draw.text((8, 8), title, fill=(255, 255, 255), font=font)

    model_text = f"Model: {model_id}"
    model_box = draw.textbbox((8, 0), model_text, font=font)
    model_height = model_box[3] - model_box[1]
    model_top = height - model_height - 12
    draw.rectangle((4, model_top - 4, model_box[2] + 8, height - 4), fill=(0, 0, 0))
    draw.text((8, model_top), model_text, fill=(255, 255, 255), font=font)
    ...
    for bbox in result.bboxes:
        ...
        draw.rectangle((left, top, right, bottom), outline=(255, 64, 64), width=4)
        label_box = draw.textbbox((left, top), bbox.label, font=font)
        label_top = max(0, top - (label_box[3] - label_box[1]) - 6)
        draw.rectangle((left, label_top, label_box[2] + 6, top), fill=(255, 64, 64))
        draw.text((left + 3, label_top + 2), bbox.label, fill=(255, 255, 255), font=font)
...
def wait_until_ready(session: requests.Session, base_url: str, model_id: str, timeout_seconds: float) -> None:
    deadline = time.monotonic() + timeout_seconds
    while time.monotonic() < deadline:
        try:
            response = session.get(base_url + "/ready", params={"model_id": model_id}, timeout=1)
...
def run_grounding(args: argparse.Namespace, task_instruction: str) -> GroundingResult:
    ...
    # The child server is always local; bypass workstation proxy settings.
    session.trust_env = False
    try:
        wait_until_ready(session, base_url, args.model_id, args.timeout_seconds)
        print("\n服务已就绪，按 TiPToP multipart/form-data 格式请求 /generate。")
        ...
        draw_grounding_result(image, result, task_instruction, args.model_id, args.result_image)
        return result
    finally:
        image.close()
        session.close()
...
def test_render_prompt_inserts_task_instruction() -> None:
    assert render_prompt('Task: "{task_instruction}". Return {{}}.', "pick up the ball") == (
...
def main() -> None:
    ...
    if task_instruction is None:
        try:
            task_instruction = input("请输入 task_instruction：").strip()
        except EOFError as exc:
            raise SystemExit("请通过 --task-instruction 提供任务指令。") from exc

    try:
        result = run_grounding(args, task_instruction)
    except (FileNotFoundError, RuntimeError, ValueError, requests.RequestException) as exc:
        raise SystemExit(f"Grounding 测试失败：{exc}") from exc
    print("\n===== OmniGround 返回结果 =====")
